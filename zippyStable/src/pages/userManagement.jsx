...
const UserManagement = () => {
    ...
    useEffect(() => {
        if (selectedStable) {
            setCenterFilter(selectedStable);
        }
    }, [selectedStable]);
    ...
    const scopedUsers = useMemo(() => {
        return users.filter(u => {
            const userStable = u.stableId || u.stable_id;
            if (userStable === selectedStable) return true;

            const isTrainerInStable = trainers.some(t => 
                (t.userId === u.id || t.user_id === u.id) && 
                (t.stableId === selectedStable || t.stable_id === selectedStable)
            );
            if (isTrainerInStable) return true;

            if (u.type === 'stableStaff') {
                const stable = stables.find(s => s.userId === u.id);
                if (stable && stable.id === selectedStable) return true;
            }
    ...
    const counts = {
        ...
        rider: scopedUsers.filter(u => u.type === 'rider').length,
        trainer: scopedUsers.filter(u => u.type === 'trainer').length,
        vet: scopedUsers.filter(u => u.type === 'vet').length,
        stableStaff: scopedUsers.filter(u => u.type === 'stableStaff').length,
    }
        ...
        <div className="p-10 max-w-[1400px] mx-auto min-h-full bg-[#fcfbf9] w-full font-sans pb-20">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center border-b border-gray-200 mb-8 mt-6 gap-4">
                <div className="flex gap-8 overflow-x-auto w-full md:w-auto no-scrollbar">
                    <TabButton type="all" label="All Users" count={counts.all - counts.stableStaff} />
