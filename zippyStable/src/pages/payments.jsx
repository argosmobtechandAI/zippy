...
const Payments = () => {
    ...
    useEffect(() => {
        setCurrentPage(1);
    }, [searchTerm, startDate, endDate, userLevelFilter, centerLevelFilter, userFilter, planFilter]);
    ...
    return (
        <div className="p-10 max-w-[1400px] mx-auto h-full overflow-y-auto w-full relative">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100/80 overflow-hidden mb-8">
                ...
                {/* Pagination Controls */}
                <div className="px-8 py-5 flex justify-between items-center border-t border-gray-100 bg-[#FAFAFA]/30 rounded-b-2xl">
                    <div className="text-[12px] font-semibold text-gray-400 tracking-wide">
                        Showing <span className="text-[#964C2E] font-bold">{filteredPayments.length === 0 ? 0 : indexOfFirstItem + 1}</span> to <span className="text-[#964C2E] font-bold">{Math.min(indexOfLastItem, filteredPayments.length)}</span> of <span className="text-[#964C2E] font-bold">{filteredPayments.length}</span> transactions
                    </div>
                    <div className="flex items-center gap-2">
                        <button 
                            disabled={currentPage === 1}
                            onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))}
                            className="w-8 h-8 rounded-lg border border-gray-200 flex items-center justify-center text-gray-400 hover:bg-gray-50 transition-colors bg-white shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                            <ChevronLeft className="w-4 h-4" />
                        </button>
                        <span className="text-sm font-bold text-gray-600 px-2">Page {currentPage} of {totalPages || 1}</span>
                        <button 
                            disabled={currentPage === totalPages || totalPages === 0}
                            onClick={() => setCurrentPage(prev => Math.min(prev + 1, totalPages))}
                            className="w-8 h-8 rounded-lg border border-gray-200 bg-white flex items-center justify-center text-[#964C2E] font-bold hover:bg-gray-50 transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
                        >
